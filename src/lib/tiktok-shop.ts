...
/** Score likely product/studio images using URL metadata as a lightweight heuristic. */
export function calculateImageQuality(url?: string | null): number {
  if (!url) return 0;

  const normalizedUrl = url.toLowerCase();
  let score = 50;
  if (normalizedUrl.includes("hero")) score += 20;
  if (normalizedUrl.includes("main") || normalizedUrl.includes("product")) score += 10;
  if (normalizedUrl.includes("studio") || normalizedUrl.includes("white")) score += 20;
