/**
 * AI content generation (captions, hashtags, image prompts, ideas).
 * Pure template engine — shared by the /api/ai/generate route and the MCP
 * `generate_caption` tool so both stay in sync.
 */

export type GenerateType = "caption" | "hashtag" | "image" | "idea";
...
const hashtagSets: Record<string, string[]> = {
  ...
  twitter: ["#{prompt} #trending #discussion"],
  linkedin: ["#{prompt} #professional #growth #insights #leadership"],
  default: [
...
export function generateContent(req: GenerateRequest) {
  const normalizedPlatform = (req.platform ?? "default").toLowerCase();
  const sanitizedPrompt = req.prompt.trim();
  ...
  switch (req.type) {
    case "caption": {
      ...
      result = { options, selected: options[0], platform: normalizedPlatform };
      break;
    }
    case "hashtag": {
      const sets =
