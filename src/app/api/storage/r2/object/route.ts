...
export async function DELETE(request: Request) {
  ...
  if (!workspaceId || !key || !(await hasWorkspaceAccess(workspaceId, session.user.id))) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  try {
    if (!await deleteR2Object(key)) return NextResponse.json({ error: "Object not found" }, { status: 404 });
    return NextResponse.json({ deleted: true, key });
  } catch (error) {
    console.error("Error deleting R2 object:", error);
    return NextResponse.json({ error: "Could not delete R2 object" }, { status: 502 });
  }
}
