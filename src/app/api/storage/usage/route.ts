...
export async function GET(request: Request) {
  ...
  const assets = await db.select({ id: mediaAssets.id, fileName: mediaAssets.fileName, fileSize: mediaAssets.fileSize, mediaType: mediaAssets.mediaType, mimeType: mediaAssets.mimeType, duration: mediaAssets.duration, createdAt: mediaAssets.createdAt }).from(mediaAssets).where(eq(mediaAssets.workspaceId, workspaceId));
