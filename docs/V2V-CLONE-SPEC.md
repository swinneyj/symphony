...
- **True-V2V tested and REJECTED:** Kling 3.0 `video_url` param passes validation echo but fails at run time (`start_image_url` required) — frame-edit is the only path; the dead direct branch was removed from the worker. falSubmit now fails fast on `{"detail":…}` bodies instead of polling a doomed job to timeout.
