...
- **MODEL PICKER (2026-08-14):** Clone tab has a model select — Kling 3.0 Pro (~$0.30, default), Kling 3.0 Standard (~$0.15, verified live), Sora 2 (OpenAI credits, verified live), Veo 3.1 (disabled — Gemini prepay credits depleted; shape verified via `:predictLongRunning` + instances/parameters).
- **FAL CREDITS PAUSED (2026-08-14):** user's fal.ai tokens depleted. Seedance 2.5 hard-gated in the worker (throws before submit — can't spend after top-up). Kling Pro/Standard + nano-banana edit are fal too → they 429 until fal is topped up; **Sora path unaffected** (OpenAI billing). Re-enable by removing the gate in `generateSeedance`.
