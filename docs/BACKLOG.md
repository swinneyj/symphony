...
| 9 | V2V Video Clone (upload source → bg-change prompt → model picker → cloned video) | Hermes | **done** | Spec: `docs/V2V-CLONE-SPEC.md`. **Shipped 2026-08-13:** worker `v2v_edit` jobType (frame-edit → Kling 3.0 Pro) + `/api/video-clone` + Clone tab in Video Studio. QA passed on a real product video (Barebells can → neon nightclub, label preserved, ~$0.30). Kling `video_url` true-V2V tested → **not supported at runtime** (start_image_url required); frame-edit is the path. Note: needs a Vercel redeploy to expose the Clone tab. |
...
| 11 | Media Downloader (paste TikTok/YT/IG link → video + MP3) | Hermes | **done** | **Shipped 2026-08-14:** `media_downloads` table + ads-worker claim loop (reuses DC-safe `download_tiktok_direct`; yt-dlp fallback for other platforms) → ffmpeg MP3 → private Blob → Downloader tab in Video Studio + `/api/media-download` (POST/GET) + session-gated file stream. **Verified live**: the spongebobprodsz mug ad downloaded in ~1.3s, video + full-length MP3 on Blob, row `done`. Blob upload from Python needed `x-vercel-blob-store-id` + `x-api-version: 12` headers (400 "Invalid pathname" without them). YT bot-walled on VPS IP (flagged in UI). |
